...
impl ChannelInterface {
    pub fn new() -> ChannelInterface {
        ChannelInterface { storage: super::storage::SingleKvStorage::new(&super::CONFIG.db_path, STORE) }
    }
    ...
    pub fn subscribe(&self, channel: &str, user: &str) -> Result<bool, &str> {
        match self.get_channel_by_id(channel) {
            Ok(mut chn) => {
                match super::user::INTERFACE.user_subscribe(user, channel) {
                    Ok(_) => {
                        chn.subscribers.push(user.to_string());
                        let json_string = serde_json::to_string(&chn).unwrap();
                        self.storage.put_single(channel, &rkv::Value::Json(&json_string));
                        Ok(true)
                    }
                    err => err
                }
    ...
    pub fn unsubscribe(&self, channel: &str, user: &str) -> Result<bool, &str> {
        match self.get_channel_by_id(channel) {
            Ok(mut chn) => {
                match super::user::INTERFACE.user_unsubscribe(user, channel) {
                    Ok(_) => {
